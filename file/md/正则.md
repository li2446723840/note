...
| ---------- | --------------------------------- |
| (?:)       | 不返回子匹配。参考案例1           |
| (?=exp)    | 匹配exp前面的位置。参考案例2      |
| $&         | 与 regexp 相匹配的子串。参考案例2 |

案例1：使用match匹配

```js
"food".match(/(foo)d/); // ['food', 'foo', index: 0, input: 'food', groups: undefined]
"food".match(/(?:foo)d/); // ['food', index: 0, input: 'food', groups: undefined]
```
案例2：数字千分位
```js
...
const res = "62723464".match(/\d{1,3}(?=(\d{3})+$)/g);
console.log(res); // ['62', '723']
```
...
```js
let phoneNum = "156****5678";
const reg1 = /(\d{3})(\*{4})(\d{4})/;
console.log(phoneNum.replace(reg1, "$10000$3")); // 15600005678
const reg2 = /\*{4}/;
console.log(phoneNum.replace(reg2, "$`")); // 1561565678
const reg3 = /\*{4}/;
console.log(phoneNum.replace(reg3, "$'")); // 15656785678
```
...
```js
let ruleRes = /^\S+(\s+\S+)*$/.test("s");
```
6、截取小数点后几位

```js
// digit：截取位数
function replaceToFixed(num, digit = 2) {
  if (isNaN(Number(num))) {
    return ""
  } else {
...
```js
const isScientificNotation = /^[+-]?\d+(.\d*)?e([+-]?\d+)$/.test("-132.3e2");
```
